...
Boolean RTPInterface::sendRTPorRTCPPacketOverTCP(u_int8_t* packet, unsigned packetSize,
  ...
  // (If the initial '$' send() succeeds, then we force the send()s for the rest of the packet to succeed, even if we have to do
  //  a blockiing send() to do so.)
  do {
    u_int8_t const dollar = '$';
    if (!sendDataOverTCP(socketNum, &dollar, 1, False)) break;
    if (!sendDataOverTCP(socketNum, (u_int8_t*)&streamChannelId, 1, True)) break;

    u_int8_t netPacketSize[2];
    netPacketSize[0] = (char) ((packetSize&0xFF00)>>8);
    netPacketSize[1] = (char) (packetSize&0xFF);
    if (!sendDataOverTCP(socketNum, netPacketSize, 2, True)) break;
